...
import Splash from '../assets/images/splash1.jpg';
import logo from '../assets/images/SMC-noBg.png';
    ...
    <div className="p-4 min-h-screen grid md:grid-cols-2 text-black">
      ...
      >
        {!token && (
          <Link
            to={button.link}
            className="absolute bg-blue-700 text-white p-4 py-2 rounded-[18px] top-4 right-4"
          >
            {button.text}
          </Link>
        )}
        <img
          alt="blank"
          src={logo}
          className="h-10 w-10 rounded-2xl bg-white bg-cover p-1"
        />
      </div>
    </div>
  );
