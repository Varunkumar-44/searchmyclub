...
const SliderClubs = ({ small, title }) => {
  ...
  return (
    <section className="w-full py-2 bg-white relative z-10 border-y-2 border-gray-200">
      <div className="container mx-auto px-4">
        ...
        >
          {brands.map((ele, i) => (
            <SwiperSlide
              key={i}
              className="flex items-center justify-center my-auto mx-16 md:px-8"
            >
              <img src={ele} alt="brand" className="w-20 md:w-24 lg:w-32" />
            </SwiperSlide>
          ))}
        </Swiper>
      </div>
